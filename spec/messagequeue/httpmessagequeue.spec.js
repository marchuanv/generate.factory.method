describe("when queuing http messages", function() {
  ...
  it("it should dequeue http response messages without error", async function() {
    ...
    // Assert
    const { httpRequestMessage } = await _httpServerMessageQueue02.dequeueHttpResponseMessage();
    expect(httpRequestMessage).not.toBeNull();
  });
});
