...
function resolveTypeInfo({ typeName, typeInfoFilePath }) {
    ...
    try {
        ...
        const childTypeInfoArray = parameters.map(param => utils.getJSONObject(typeInfoTemplate
            .replace(/\[TypeName\]/g, param.name)
            .replace(/\[IsSingleton\]/g, false)
            .replace(/\[IsContextSingleton\]/g, false)
            .replace(/\[ChildTypeInfoArray\]/g,'')
        ...
        if (!childTypeInfoArray || utils.isEmptyObject(childTypeInfoArray)) {
        ...
        parentInfo = utils.getJSONObject(typeInfoTemplate
            ...
            .replace(/\[ChildTypeInfoArray\]/g, childTypeInfoArray.map(child => utils.getJSONString(child)).join(','))
    ...
    for(const unresolvedChildInfo of parentInfo.childTypeInfoArray.filter(cInf => !cInf.IsResolved)) {
        const unresolvedChildInfoIndex = parentInfo.childTypeInfoArray.findIndex(cInfo => cInfo.typeName.toLowerCase() === unresolvedChildInfo.typeName.toLowerCase());
        ...
        if (unresolved) {
            {
                ...
                parentInfo.childTypeInfoArray[unresolvedChildInfoIndex] = resolveTypeInfo({ typeName, typeInfoFilePath });
            }
        }
    };
    return parentInfo;
...
module.exports = {
    ...
    resolve: ({ prototypeScriptPath, typeInfoOutputDirPath }) => {
        ...
        const knownTypeInfo = typeInfo.find(tl => tl.typeName === typeName);
        if (!knownTypeInfo) {
            throw new Error('add type info first');
        }
    ...
    get: ({ typeInfoOutputDirPath, typeName }) => {
        const typeInfoFilePath = path.join(typeInfoOutputDirPath, 'type.info.json');
        const typeInfo = loadTypeInfo({ typeInfoFilePath });
        return typeInfo.find(t => t.typeName.toLowerCase() === typeName.toLowerCase());
    }
}
