...
function getTypeInfoTree({ type, isSingleton, unresolvedTypeLibrary }) {
    ...
    for(const unresolvedChildInfo of parentInfo.childInfoArray.filter(cInf => !cInf.IsResolved)) {
        ...
        const { type, isSingleton } = unresolvedTypeLibrary.findIndex(info => info.typeName.toLowerCase() === unresolvedChildInfo.typeName.toLowerCase());
...
module.exports = {
    ...
    get: ({ typeName }) => {
        ...
        const { type, isSingleton } = typeLibrary.find(tl => tl.typeName === typeName);
