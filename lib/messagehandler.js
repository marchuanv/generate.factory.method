...
function MessageHandler({ httpMessageHandler, webSocketMessageHandler, httpMessageFactory, webSocketMessageFactory }) {
    ...
    if (httpMessageHandler) {
        Object.defineProperty(this, 'send', { writable: false, value: async ({ host, port, data }) => {
            const httpResponseMessage = await httpMessageHandler.send({ host, port, data });
            return { message: httpMessageFactory.convertHttpResponseMessageToMessage({ httpResponseMessage })};
        }});
        Object.defineProperty(this, 'receive', { writable: false, value: ({ host, port, callback }) => {
            httpMessageHandler.receive({ host, port, callback: async ({ httpRequestMessage }) => {
        ...
        Object.defineProperty(this, 'send', { writable: false, value: async ({ host, port, data }) => {
            return await webSocketMessageHandler.send({ host, port, data });
        }});
        Object.defineProperty(this, 'receive', { writable: false, value: ({ host, port, callback }) => {
            webSocketMessageHandler.receive({ host, port, callback: async ({ message }) => {
                return await callback({ message });
            }});
        }});
    }
}
MessageHandler.prototype.send = async function({ host, port, data }) { };
MessageHandler.prototype.receive = function({ host, port, callback }) { };
