...
function HttpConnection({ httpRequestQueue, httpResponseQueue, hostAddress, timeout }) {
    ...
    Object.defineProperty(this, 'send', { writable: false, value: ({ address, path, headers, method, data }) => {
        return new Promise((resolve, reject) => {
            const httpRequest = http.request({ hostname: address.address, post: address.port, path, headers, method });
    ...
    Object.defineProperty(this, 'open', { writable: false, value: () => {
        return new Promise((resolve) => {
            ...
            server.on("request", async (httpRequest, httpResponse) => {
                ...
                httpResponse.Id = utils.generateGUID();
                httpRequest.on('error', (error) => {
                    httpRequestQueue.enqueue({ httpRequest, error });
                    httpResponseQueue.enqueue({ httpResponse, error });
                });
                httpRequest.on('data', (chunk) => httpRequest.body += chunk );
                httpRequest.on('end', () => {
                    httpRequestQueue.enqueue({ httpRequest });
                    httpResponseQueue.enqueue({ httpResponse });
                });
            });
            server.on("listening", () => {
