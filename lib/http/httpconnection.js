...
const waitForClientHttpRequests = async ({ messageQueue, timeout  }) => {
    ...
    const httpRequest = http.request({host: senderHost, port: senderPort, path, headers, method });
    httpRequest.Id = id;
...
function HttpConnection({ messageQueue, hostAddress, timeout }) {
    waitForClientHttpRequests({ messageQueue, timeout });
    let server;
    Object.defineProperty(this, 'open', { configurable: false, writable: false, value: () => {
        return new Promise((resolve) => {
            ...
            server.on("request", async (httpRequest, httpResponse) => {
                ...
                httpRequest.body = '';
                httpRequest.Id = id;
                httpRequest.senderHost = hostAddress.host;
                httpRequest.senderPort = hostAddress.port;

                httpResponse.Id = id;
                httpResponse.senderHost = hostAddress.host;
                httpResponse.senderPort = hostAddress.port;
