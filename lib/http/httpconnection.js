...
async function waitForClientHttpRequests({ httpClientMessageQueue, timeout, isOpen }) {
    ...
    if (!isOpen()) {
        return await httpClientMessageQueue.enqueueHttpRequestMessage({ requestMessage: httpRequestMessage });
    }
        ...
        return new Promise((resolve) => {
            ...
            server.on("request", async (httpRequest, httpResponse) => {
                ...
                httpRequest.on('end', async () => {
                    ...
                    if (!this.isOpen()) {
                        return await httpServerMessageQueue.enqueueHttpResponseMessage({ responseMessage: httpResponseMessage });
                    }
