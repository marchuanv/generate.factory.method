...
const waitForClientHttpRequests = async ({ messageQueue, timeout  }) => {
    ...
    const { host, port } = httpRequestMessage.getSenderAddress();
    ...
    httpRequest.on('response', (httpResponse) => {
        ...
        httpResponse.on('end', () => {
            messageQueue.enqueueHttpResponseMessage({ httpResponse });
        });
    });
        ...
        return new Promise((resolve) => {
            ...
            server.on("request", async (httpRequest, httpResponse) => {
                ...
                httpRequest.on('end', async () => {
                    ...
                    messageQueue.enqueueHttpRequestMessage({ httpRequest });
