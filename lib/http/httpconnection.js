...
const waitForClientHttpRequests = async ({ httpClientMessageQueue, timeout }) => {
...
function HttpConnection({ httpClientMessageQueue, httpServerMessageQueue, senderAddress, timeout }) {
    waitForClientHttpRequests({ httpClientMessageQueue, timeout });
    let server;
    Object.defineProperty(this, 'open', { configurable: false, writable: false, value: () => {
        return new Promise((resolve) => {
            const host = senderAddress.address || senderAddress.host;
            const port = senderAddress.port;
