...
function HttpServerMessageQueue({ sharedMessageQueue, userId, senderAddress, recipientAddress }) {
    ...
    Object.defineProperty(this, 'enqueueHttpRequest', { configurable: false, writable: false, value: async ({ httpRequest }) => {
        ...
        const { queueId } = await sharedMessageQueue.queueMessage({ message: httpRequestMessage, messageQueueType: 'HttpServerRequestMessage' });
        return { queueId };
    }});
    Object.defineProperty(this, 'enqueueHttpResponseMessage', { configurable: false, writable: false, value: async ({ responseMessage }) => {
        ...
        const { queueId } = await sharedMessageQueue.queueMessage({ message: httpResponseMessage, messageQueueType: 'HttpServerResponseMessage' });
        return { queueId };
    }});
    Object.defineProperty(this, 'dequeueHttpRequestMessage', { configurable: false, writable: false, value: async () => {
        const { message, queueId }  = await sharedMessageQueue.dequeueMessage({ messageQueueType: 'HttpServerRequestMessage' });
    ...
    Object.defineProperty(this, 'dequeueHttpResponseMessage', { configurable: false, writable: false, value: async () => {
        const { message, queueId }  = await sharedMessageQueue.dequeueMessage({ messageQueueType: 'HttpServerResponseMessage' });
