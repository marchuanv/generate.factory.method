...
function HttpServerMessageQueue({ userId, senderAddress, recipientAddress }) {
    ...
    Object.defineProperty(this, 'sync', { configurable: false, writable: false, value: ({ httpServerMessageQueue }) => {
        ...
        setInterval(() => {
            const httpRequestMessagesCloned = httpServerMessageQueue.cloneRequestMessages();
            httpRequestMessages = httpRequestMessages.concat(httpRequestMessagesCloned);
            const httpResponseMessagesCloned = httpServerMessageQueue.cloneResponseMessages();
