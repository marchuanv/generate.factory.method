...
function HttpRequestQueue() {
    ...
    Object.defineProperty(this, 'enqueue', { writable: false, value: ({ httpRequest, isClient, error }) => {
        ...
        if (clientQueue.find(q => q.httpRequest.Id === httpRequest.Id )) {
            console.log('http server response queued.');
            if (isClient) {
                clientQueue.push({ httpRequest, error });
            } else {
                serverQueue.push({ httpRequest, error });
            }
        }
        if (clientQueue.find(q => q.httpRequest.Id === httpRequest.Id )) {
    ...
    Object.defineProperty(this, 'dequeue', { writable: false, value: ({ isClient }) => {
        return new Promise((resolve) => {
            const id = setInterval(() => {
                const { httpRequest, error } = isClient ? clientQueue.shift() : serverQueue.shift();
                if (error) {
                    return reject(error);
                }
                if (httpRequest) {
