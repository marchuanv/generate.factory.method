...
function HttpMessageBus({ httpClientMessageQueue, httpServerMessageQueue, senderAddress, timeout }) {
    ...
    {
        const { eventSubscription } = createEventSubscription({ eventCode: 4, subscriptionName: 'HttpClientMessageQueue' });
    ...
    { 
        const { eventSubscription } = createEventSubscription({ eventCode: 1, subscriptionName: 'HttpClientMessageQueue' });
        eventSubscription.subscribe({ callback: () => {
            ...
            server.on("listening",async () => {
                const { eventPublisher } = createEventPublisher({ eventCode: 3, eventSource: 'HttpMessageBus', eventDescription: `listening on: ${utils.getJSONString(options)}` });
                eventPublisher.publish();
            });
    ...
    {
        const { eventSubscription } = createEventSubscription({ eventCode: 8, subscriptionName: 'HttpClientMessageQueue' });
        eventSubscription.subscribe({ callback: async () => {
            await server.close();
            const { eventPublisher } = createEventPublisher({ eventCode: 9, eventSource: 'HttpMessageBus', eventDescription: 'http server messagebus has stopped litening' });
