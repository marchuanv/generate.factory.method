...
function HttpMessageBus({ httpClientMessageQueue, httpServerMessageQueue, senderAddress, timeout, contextId }) {
    let server;
    ...
    {
        const { eventSubscription } = createEventSubscription({ contextId, eventCode: 4, subscriptionName: 'HttpClientMessageQueue' });
        eventSubscription.subscribe({ callback: async () => {
            ...
            httpRequest.on('response', (httpResponse) => {
                ...
                httpResponse.on('error', (error) => {
                    const { eventPublisher } = createEventPublisher({ contextId, eventCode: 5, eventSource: 'HttpMessageBus', eventDescription: error });
            ...
            httpRequest.on('error',(error) => { 
                const { eventPublisher } = createEventPublisher({ contextId, eventCode: 6, eventSource: 'HttpMessageBus', eventDescription: error });
                eventPublisher.publish();
            });
    ...
    { 
        const { eventSubscription } = createEventSubscription({ contextId, eventCode: 1, subscriptionName: 'HttpClientMessageQueue' });
        eventSubscription.subscribe({ callback: () => {
            ...
            server.on("error", async (error) => {
                if (host){
                    dns.lookup(host, async (dnsErr) => {
                        if (dnsErr){
                            const { eventPublisher } = createEventPublisher({ contextId, eventCode: 7, eventSource: 'HttpMessageBus', eventDescription: dnsErr });
                            eventPublisher.publish();
                        }
                    });
                } else {
                    const { eventPublisher } = createEventPublisher({ contextId, eventCode: 7, eventSource: 'HttpMessageBus', eventDescription: error });
                    eventPublisher.publish();
                }
            });
            server.on("request", (httpRequest, httpResponse) => {
                ...
                httpRequest.body = '';
                httpRequest.on('error', (error) => {
                    const { eventPublisher } = createEventPublisher({ contextId, eventCode: 2, eventSource: 'HttpMessageBus', eventDescription: error });
                    eventPublisher.publish();
                });
            ...
            server.on("listening",async () => {
                const { eventPublisher } = createEventPublisher({ contextId, eventCode: 3, eventSource: 'HttpMessageBus', eventDescription: `listening on: ${utils.getJSONString(options)}` });
                eventPublisher.publish();
            });
    ...
    {
        const { eventSubscription } = createEventSubscription({ contextId, eventCode: 8, subscriptionName: 'HttpClientMessageQueue' });
        eventSubscription.subscribe({ callback: async () => {
            await server.close();
            const { eventPublisher } = createEventPublisher({ contextId, eventCode: 9, eventSource: 'HttpMessageBus', eventDescription: 'http server messagebus has stopped litening' });
