...
function HttpMessageBus({ httpClientMessageQueue, httpServerMessageQueue, senderAddress, timeout }) {
    ...
    {
        const { eventSubscription } = createEventSubscription({ eventCode: 4, eventSource: 'HttpMessageBus' });
    ...
    { 
        const { eventSubscription } = createEventSubscription({ eventCode: 1, eventSource: 'HttpMessageBus', eventDescription: 'Start, Listen For And Respond to Http Requests' });
    ...
    {
        const { eventSubscription } = createEventSubscription({ eventCode: 8, eventSource: 'HttpMessageBus', eventDescription: 'Stop Http Messagebus Server' });
        eventSubscription.subscribe({ callback: async () => {
            await server.close();
