...
HttpClientMessageBus.prototype.constructor = function({ 
    ...
    httpClientStopMessageQueueBinding.dequeueMessage().then(({ message }) => {
        if (message.contextName === contextName) {
    ...
    httpClientStartMessageQueueBinding.dequeueMessage().then(() => {
        started = true;
        httpClientStartedMessageQueueBinding.queueMessage({ message: { contextName } });
        const intervalId = setInterval( async ()=> {
            ...
            httpRequest.on('error', (error) => {
                httpClientStopMessageQueueBinding.queueMessage({ message: { contextName } });
                console.log(error);
            });
