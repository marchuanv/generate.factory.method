...
function HttpClientMessageBus({ messageQueue, messageConverter, logger, timeout }) {
    const { createHttpResponseMessage } = require('../factory/httpresponsemessage.factory');
    const responseQueueName = 'HttpClientResponses';
    const requestQueueName = 'HttpClientRequests';
        ...
        messageQueue.dequeueMessage({ queueName: requestQueueName }).then( async ({ message }) => {
            ...
            httpRequest.on('response', (httpResponse) => {
                ...
                httpResponse.on('end', async () => {
                    ...
                    const { httpResponseMessage } = createHttpResponseMessage({ scopeId: utils.generateGUID(), messageStatusCode, Id: null, data,
