...
function HttpResponseQueue() {
    ...
    Object.defineProperty(this, 'enqueue', { writable: false, value: ({ httpResponse, isClient, error }) => {
        ...
        if (clientQueue.find(q => q.httpResponse.Id === httpResponse.Id )) {
            console.log('http server response queued.');
            if (isClient) {
                clientQueue.push({ httpResponse, error });
            } else {
                serverQueue.push({ httpResponse, error });
            }
        }
        if (clientQueue.find(q => q.httpResponse.Id === httpResponse.Id )) {
            console.log('http client response queued.');
            if (isClient) {
                clientQueue.push({ httpResponse, error });
            } else {
                serverQueue.push({ httpResponse, error });
    ...
    Object.defineProperty(this, 'dequeue', { writable: false, value: ({ isClient }) => {
        return new Promise((resolve, reject) => {
            const id = setInterval(() => {
                const { httpResponse, error } = isClient ? clientQueue.shift() : serverQueue.shift();
                if (error) {
                    return reject(error);
                }
                if (httpResponse) {
