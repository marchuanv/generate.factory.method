...
function HttpServerMessageBus({ sharedMessageQueue, sharedLogging, senderAddress, timeout, contextId }) {
    const getQueueNames = 'getQueueNames';
    sharedMessageQueue.bind({ queueName: getQueueNames });
    sharedMessageQueue.dequeueGlobalMessage().then(({ message }) => {
        ...
        server.on("request", (httpRequest, httpResponse) => {
            ...
            httpRequest.on('end', () => {
                sharedMessageQueue.dequeueMessage({ queueName: getQueueNames }).then( async ({ message }) => {
    ...
    sharedMessageQueue.queueMessage({ message: { requestQueueName, responseQueueName }, queueName: getQueueNames });
