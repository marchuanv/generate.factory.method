...
function HttpServerMessageBus({ sharedMessageQueue, sharedLogging, senderAddress, timeout, contextId }) {
    const switchContextQueueName = 'switchservercontext';
    sharedMessageQueue.bind({ queueName: switchContextQueueName });
    sharedMessageQueue.dequeueGlobalMessage().then(({ message }) => {
        
        if (!message.startserver) {
            sharedLogging.log({ date: new Date(), context: 'HttpServer', text: 'failed to start http server' });
            return;
        }
        ...
        server.on("request", (httpRequest, httpResponse) => {
            ...
            httpRequest.on('end', () => {
                sharedMessageQueue.dequeueMessage({ queueName: switchContextQueueName }).then( async ({ message }) => {
    ...
    sharedMessageQueue.queueMessage({ message: { requestQueueName, responseQueueName }, queueName: switchContextQueueName });
