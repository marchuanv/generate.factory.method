...
function HttpServerMessageBus({ senderAddress, messageQueue, logger, timeout }) {
    ...
    const startstopServer = () => {
        messageQueue.queueGlobalMessage({ message: { startserver } });
        messageQueue.dequeueGlobalMessage().then(({ message }) => {
