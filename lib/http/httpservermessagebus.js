...
HttpServerMessageBus.prototype.constructor = function({ 
    ...
    httpServerStartMessageQueueBinding.dequeueMessage().then(({ message }) => {
        if (message.factoryContainerBindingName !== message.factoryContainerBindingName) {
        ...
        server.on("error", (error) => {
            ...
            httpServerStopMessageQueueBinding.queueMessage({ message: { factoryContainerBindingName } });
        ...
        server.on("listening", () => {
            ...
            httpServerStartedMessageQueueBinding.queueMessage({ message: { factoryContainerBindingName } });
        });
        server.on('close', () => {
            logger.log({ date: new Date(), context: 'HttpServer', text: 'server was stopped' });
            httpServerRequestMessageQueueBinding.unbind();
            httpServerResponseMessageQueueBinding.unbind();
            httpServerStoppedMessageQueueBinding.queueMessage({ message: { factoryContainerBindingName } });
        });
