...
function HttpServerMessageBus({ sharedMessageQueue, senderAddress, timeout, contextId }) {
    ...
    const serverQueueName = `HttpServer`;
    sharedMessageQueue.bind({ queueName: responseQueueName });
    sharedMessageQueue.bind({ queueName: requestQueueName });
    if (!sharedMessageQueue.isBinded({ queueName: serverQueueName })) {
        sharedMessageQueue.bind({ queueName: serverQueueName });
    }
    sharedMessageQueue.dequeueMessage({ queueName: serverQueueName }).then(() => { //ONLY WANT THIS MESSAGE TO HAPPEN ONCE!
    ...
    Object.defineProperty(this, 'publishHttpResponseMessage', { configurable: false, writable: false, value: async ({ httpResponseMessage }) => {
        await sharedMessageQueue.queueMessage({ message: { text: 'ensureserverstarted' }, queueName: serverQueueName });
