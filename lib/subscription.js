function Subscription({ channelName }) {
    const callbacks = [];    
    Object.defineProperty(this, 'getChannelName', { configurable: false, writable: false, value: () => channelName });
    Object.defineProperty(this, 'receiveData', { configurable: false, writable: false, value: async ({ senderHost, senderPort, data }) => {
        for(const callback of callbacks) {
            await callback({ senderHost, senderPort, data });
        };
    }});
    Object.defineProperty(this, 'onDataReceived', { onfigurable: true, writable: false, value: ({ callback }) => {
        callbacks.push(async ({ senderHost, senderPort, data }) => callback({ senderHost, senderPort, data }));
    }});
}
Subscription.prototype.getChannelName = () => {};
Subscription.prototype.receiveData = ({ senderHost, senderPort, data }) => { }
Subscription.prototype.onDataReceived = ({ callback }) => { }
module.exports = { Subscription };
