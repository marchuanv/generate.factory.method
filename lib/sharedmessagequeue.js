...
function SharedMessageQueue() {
    ...
    Object.defineProperty(this, 'queueMessage', { configurable: false, writable: false, value: async ({ message, messageQueueType, messageQueueContextId }) => {
        console.log(`queuing 1 message onto the ${messageQueueType} queue.`);
        const binding = bindings[messageQueueType][messageQueueContextId];
        binding.messages.push({ message });
        await dequeue({ messageQueueType });
    }});
    Object.defineProperty(this, 'dequeueMessage', { configurable: false, writable: false, value: ({ messageQueueType, messageQueueContextId }) => {
    ...
    Object.defineProperty(this, 'bind', { configurable: false, writable: false, value: ({ messageQueueType, messageQueueContextId}) => {
        const binding = bindings[messageQueueType][messageQueueContextId];
        if (binding) {
            if (binding.callbacks.length > 0) {
                if (binding.waitCount  === 3) {
                    ...
                    binding.locked = true;
                    setTimeout(() => this.bind({ messageQueueType }),1000);
                }
                return;
            }
        } else {
            bindings[messageQueueType] = { messageQueueContextId: { locked : false, callbacks: [], messages: [], waitCount: 0 } };
        }
    }});
};
