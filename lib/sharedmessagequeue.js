...
function SharedMessageQueue({ sharedLogger }) {
    ...
    const dequeue = async ({ queueName }) => {
        ...
        if (binding.locked) {
            sharedLogger.log({ date: new Date(), context: queueName, text: `'${queueName}' is waiting for queue lock.` });
        ...
        if (callback && !callback.resolved) {
            ...
            sharedLogger.log({ date: new Date(), context: queueName, text: `message pulled from the '${queueName}' queue.` });
    ...
    Object.defineProperty(this, 'queueMessage', { configurable: false, writable: false, value: async ({ message, queueName }) => {
        sharedLogger.log({ date: new Date(), context: queueName, text: `message published on the '${queueName}' queue.` });
        const binding = bindings[queueName];
    ...
    Object.defineProperty(this, 'queueGlobalMessage', { configurable: false, writable: false, value: async ({ message }) => {
        sharedLogger.log({ date: new Date(), context: globalQueueName, text: `message published on the '${globalQueueName}' global queue.` });
...
SharedMessageQueue.prototype.clearQueueMessages = function ({ queueName }) { };
