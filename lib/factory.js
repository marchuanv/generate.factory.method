...
function Factory({ typeName, typeVariableName, typeScriptPath, isSingleton, bindings }) {
    Object.defineProperty(this, 'getInstance', { configurable: false, writable: false, value: ({ factoryContainerBindingName, ctorArgs }) => {
        ...
        let { factoryContainerBindingScriptPath, primitiveArgs, referenceArgs, instance } = binding;
        if (factoryContainerBindingScriptPath) {
            if (!existsSync(factoryContainerBindingScriptPath)) {
                throw new Error(`"${factoryContainerBindingScriptPath}" script does not exist.`);
            }
            binding = require(factoryContainerBindingScriptPath);
