...
function Factory() {
    const allConfig = utils.getJSONObject(utils.getJSONString(factoryConfig));
    function getCtorParameters({ type, isImmediate, isReference }) {
        const ctorParams = {};
        if (typeof type !== 'function') {
            throw new Error("the 'type' parameter is not of type: Function");
        }
        const config = allConfig.find(cnf => cnf.name === type.name);
        ...
        if (!isImmediate){
            for(const dep of allConfig.filter(cnf => config.parameters.find(p => p.name.toLowerCase() === cnf.variableName))) {
    ...
    function getInstance({ config, otherCtorParams }) {
        const depConfigArray = allConfig.filter(cnf => config.parameters.find(p => p.name.toLowerCase() === cnf.variableName) !== undefined);
