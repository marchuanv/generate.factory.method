...
for(const config of factoryConfig) {
    ...
    const depConfigArray =  factoryConfig.filter(cnf => config.parameters.find(p => p.name.toLowerCase() === cnf.variableName));
    const ctorParams = Object.keys(getCtorParameters({ type, isImmediate: false, isReference: false })).reduce((newObj, key) => {
