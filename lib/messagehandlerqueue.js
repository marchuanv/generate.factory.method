function MessageHandlerQueue({ sharedMessageQueue, messageQueueContextId }) {
    ...
    {
        const { eventSubscription } = createEventSubscription({ eventCode: 3, subscriptionName: 'MessageHandlerQueue' });
    ...
    {
        const { eventSubscription } = createEventSubscription({ eventCode: 9, subscriptionName: 'MessageHandlerQueue' });
        eventSubscription.subscribe({ callback: () => {
