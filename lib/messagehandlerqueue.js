function MessageHandlerQueue({ sharedMessageQueue, messageQueueContextId }) {
    ...
    {
        const { eventSubscription } = createEventSubscription({ messageQueueContextId, eventCode: 3, subscriptionName: 'MessageHandlerQueue' });
    ...
    {
        const { eventSubscription } = createEventSubscription({ messageQueueContextId, eventCode: 9, subscriptionName: 'MessageHandlerQueue' });
        eventSubscription.subscribe({ callback: () => {
