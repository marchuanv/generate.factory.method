...
function ContentMetadata({ data, metadata }) {
    let name = null;
    let encrypted = false;
    const object = utils.getJSONObject(data);
    if (object) {
     name = 'json';
     this.sender = metadata.sender || object.sender;
     this.token = metadata.token || 'INVALID';
     data = utils.getJSONString(data);
...
ContentMetadata.prototype.isEncrypted = function() {}
