...
function MessageQueue({ factoryContainer, messageQueueRegistry, logger }) {
    
    let callbacks = [];
    let messages = [];
    const bindings = {};

    for(const type of messageQueueRegistry.getTypes()) {
        const bindingName = `${factoryContainer.getId()}_${type.name}`;
        bindings[bindingName] = {};
    }

    const ensureBindings = async ({ binding }) => {
        bindings.locked = true;
        const isBinding = binding.name === 'binding';
        const messagesThatMatchQueue = messages.filter(m => m.queueName === binding.name);
        binding.messages = binding.messages.concat(messagesThatMatchQueue);
        ...
        if (message) {
            ...
            if (isBinding) {
                log({ logger, queueName: _queueName, text: 'binding' });
                if (!bindings[_queueName]) {
                    bindings[_queueName] = { callbacks: [], messages: [] };
                }
            } else {
                log({ logger, queueName: _queueName, text: 'unbinding.' });
                delete bindings[_queueName];
            }
            ensureBindings({ binding });
        } else {
            bindings.locked = false;
        }
    }
    ...
    Object.defineProperty(this, 'queueMessage', { configurable: false, writable: false, value: async ({ message, queueName }) => {
        ...
        ensureBindings({ binding: bindings['binding'] });
        ensureBindings({ binding: bindings['unbinding'] });
        dequeue({ queueName });
    }});
