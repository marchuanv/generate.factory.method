...
/**
* Create MessageBus
* @param {token,recipientHost,recipientPort,userId,senderHost,senderPort,Id,data,metadata,messageStatusCode,channelName}
*/
function createMessageBus({token,recipientHost,recipientPort,userId,senderHost,senderPort,Id,data,metadata,messageStatusCode,channelName}) {
    const container = factory.createContainer({ type: MessageBus, variableName:'messageBus', singleton: false });
    container.config({token,recipientHost,recipientPort,userId,senderHost,senderPort,Id,data,metadata,messageStatusCode,channelName});
