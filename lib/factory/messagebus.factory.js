...
/**
* Create MessageBus
* @param {recipientHost,recipientPort,messageQueueContextId,channel,senderHost,senderPort}
*/
function createMessageBus({recipientHost,recipientPort,messageQueueContextId,channel,senderHost,senderPort}) {
    const container = factory.createContainer({ type: MessageBus, variableName:'messageBus', singleton: false });
    container.config({recipientHost,recipientPort,messageQueueContextId,channel,senderHost,senderPort});
