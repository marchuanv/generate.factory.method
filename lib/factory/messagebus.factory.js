...
function createMessageBus({host,port,userId,channelName}) {
    let factoryContainer = new FactoryContainer();
    factoryContainer.add({host,port,userId,channelName});
    factoryContainer.add(createSubscription({channelName}));
factoryContainer.add(createMessageHandler({userId}));
