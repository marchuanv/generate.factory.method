...
/**
* Create MessageBus
* @param {recipientHost,recipientPort,contextId,channel,token,senderHost,senderPort}
*/
function createMessageBus({recipientHost,recipientPort,contextId,channel,token,senderHost,senderPort}) {
    const container = factory.createContainer({ type: MessageBus, variableName:'messageBus', singleton: false });
    container.config({recipientHost,recipientPort,contextId,channel,token,senderHost,senderPort});
    container.config(createSenderAddress({senderHost,senderPort}));
container.config(createSharedUserSessions({}));
container.config(createMessageHandlerQueue({contextId}));
