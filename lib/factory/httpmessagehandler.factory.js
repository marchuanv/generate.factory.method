...
function createHttpMessageHandler({messageQueueTypeCode,recipientHost,recipientPort,userId,senderHost,senderPort}) {
    ...
    factoryContainer.add({messageQueueTypeCode,recipientHost,recipientPort,userId,senderHost,senderPort});
    factoryContainer.add(createHttpClientMessageQueue({recipientHost,recipientPort,messageQueueTypeCode,userId,senderHost,senderPort}));
factoryContainer.add(createHttpServerMessageQueue({recipientHost,recipientPort,messageQueueTypeCode,userId,senderHost,senderPort}));
factoryContainer.add(createMessageQueue({messageQueueTypeCode}));
