...
* @param {recipientHost,recipientPort,metadata,userId,senderHost,senderPort}
*/
function createMessageMetadata({recipientHost,recipientPort,metadata,userId,senderHost,senderPort}) {
    const container = factory.createContainer({ type: MessageMetadata, variableName:'messageMetadata', singleton: false });
    container.config({recipientHost,recipientPort,metadata,userId,senderHost,senderPort});
