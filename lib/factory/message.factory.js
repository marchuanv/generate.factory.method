...
/**
* Create Message
* @param {recipientHost,recipientPort,Id,data,metadata,messageStatusCode,senderHost,senderPort}
*/
function createMessage({recipientHost,recipientPort,Id,data,metadata,messageStatusCode,senderHost,senderPort}) {
    const container = factory.createContainer({ type: Message, variableName:'message', singleton: false });
    container.config({recipientHost,recipientPort,Id,data,metadata,messageStatusCode,senderHost,senderPort});
...
container.config(createMessageContent({data,metadata}));
