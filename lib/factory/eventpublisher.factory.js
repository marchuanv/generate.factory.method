...
* @param {messageQueueContextId,eventDescription,eventCode,eventSource}
*/
function createEventPublisher({messageQueueContextId,eventDescription,eventCode,eventSource}) {
    const container = factory.createContainer({ type: EventPublisher, variableName:'eventPublisher', singleton: false });
    container.config({messageQueueContextId,eventDescription,eventCode,eventSource});
